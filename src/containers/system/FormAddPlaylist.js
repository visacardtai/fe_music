...
const FormAddPlaylist = () => {
  const { adminaddplaylist } = useSelector((state) => state.music);
  ...
  const handleAdd = () => {
    try {
      if (name === "" || title === "" || date === "") {
        toast.warn("Name, Title và Date không để trống !!!");
      } else {
        if (file0) {
          ...
          fileRef.put(file0).then((snapshot) => {
            snapshot.ref.getDownloadURL().then((url) => {
              const fetchAdd = async () => {
                const response = await apis.addAPlaylist(
                  name,
                  title,
                  url,
                  date
                ...
                if (response?.status === 200) {
                  ...
                  dispatch(actions.adminaddplaylist(false));
                } else {
